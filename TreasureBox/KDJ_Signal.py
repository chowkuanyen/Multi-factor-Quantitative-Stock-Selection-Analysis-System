...
print(f"✅ 共 {len(highlight_map)} 个信号点可用于高亮（近30天，且通过趋势验证）")
...
wb = Workbook()
ws = wb.active
ws.title = "Stock Close & KDJ Signal"

# ——————————————————————————————————————
# 12.1 插入标题行
# ——————————————————————————————————————
ws.insert_rows(1)
title_cell = ws.cell(row=1, column=1, value="股票收盘价与KDJ信号聚焦报告（近30天，仅展示有信号且价格持续上涨的股票）")
title_cell.font = Font(bold=True, size=16, color="2E5488")
# 理论值基于国泰君安证券研究所《技术分析因子有效性评估报告》（2023年内部报告）
title_cell.alignment = Alignment(horizontal="center", vertical="center")
ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=len(trade_dates) + 1)
ws.row_dimensions[1].height = 35

# ——————————————————————————————————————
# 12.2 插入前置说明（关键：放在表头上方，醒目提示）
# ——————————————————————————————————————
ws.insert_rows(2)
note_cell = ws.cell(row=2, column=1, value="🔴 红色背景单元格：表示该股票在该日触发了 KDJ 买入/卖出信号，且当日有有效收盘价。"
                                           "\n✅ 仅保留‘信号日收盘价 < 最近交易日收盘价’的股票，确保信号伴随上涨动能。")
note_cell.font = Font(bold=True, color="FF0000", size=12)
note_cell.alignment = Alignment(horizontal="left", vertical="top")
ws.merge_cells(start_row=2, start_column=1, end_row=3, end_column=len(trade_dates) + 1)
ws.row_dimensions[2].height = 40

# ——————————————————————————————————————
# 12.3 写入表头：第一列是“股票名称 (代码)”，后续是交易日期
# ——————————————————————————————————————
ws.cell(row=4, column=1, value="Stock Name (Code)")
for col_idx, date_str in enumerate(trade_dates, 2):
    ws.cell(row=4, column=col_idx, value=date_str)

# ——————————————————————————————————————
# 12.4 写入数据：每只有效股票一行，展示所有交易日的 close
# ——————————————————————————————————————
row_idx = 5
for stock_code in final_effective_stock_codes:
    ...
    for col_idx, date_str in enumerate(trade_dates, 2):
        ...
        # ✅ 如果该股票在该日有信号，高亮
        if highlight_map.get((symbol, date_str), False):
...
# 12.6 保存文件
# ——————————————————————————————————————
output_file = "stock_close_kdj_signal_report.xlsx"
wb.save(output_file)
